...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createClient() {
    ...
    const body = { ...req.body };
    if (body?.notion_template?.length > 0) {
      const create_notion_template = body.notion_template;
      body.notion_template = {
        create: create_notion_template,
      };
    } else {
      delete body.notion_template;
    }
